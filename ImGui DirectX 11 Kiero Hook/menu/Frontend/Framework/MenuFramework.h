...
typedef ImColor D3DCOLOR;
typedef ImColor D3DCOLOR_RGBA;
...
namespace IdaLovesMe {
	...
	public:
		...
		void AddSprite(ID3D11ShaderResourceView* Texture, Vec2 Pos, Vec2 Size, ImColor Color);
	...
	namespace ui {
		...
		void					 GetInputFromWindow(HWND window);
		...
		void TabButtonFiller(const char* label, int* selected, int num, int total, GuiFlags flags = 0);
		...
		bool					 SingleSelect(const char* label, int* current_item, std::vector<std::string> items);
