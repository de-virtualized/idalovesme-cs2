...
void ui::GetInputFromWindow(HWND window) {
	GuiContext& g = *Gui_Ctx;
	/*
	POINT p_mouse_pos;
	GetCursorPos(&p_mouse_pos);
	ScreenToClient(window, &p_mouse_pos);*/

	for (int i = 0; i < 256; i++) {
		if (i == VK_INSERT)
		{
			g.PrevKeyState[i] = false;
			g.KeyState[i] = false;
			continue;
		}
	...
	g.MousePos = Vec2{ ImGui::GetIO().MousePos.x, ImGui::GetIO().MousePos.y };

	g.MouseWheel = ImGui::GetIO().MouseWheel;
}
...
bool ui::ButtonBehavior(GuiWindow* Window, const char* label, Rect bb, bool& hovered, bool& held, GuiFlags flags) {
	...
	hovered = IsInside(current_window->Pos.x, current_window->Pos.y, current_window->Size.x, current_window->Size.y) && IsInside(bb.Min.x, bb.Min.y, bb.Max.x, bb.Max.y) && (*selected_item == "" || *selected_item == label);
...
void ui::BeginChild(const char* Name, Rect X, GuiFlags flags) {
	...
	ImGui::GetForegroundDrawList()->PushClipRect(ImVec2(ClipRect.left, ClipRect.top), ImVec2(ClipRect.right, ClipRect.bottom), false);
...
void ui::EndChild() {
	...
	Render::Draw->Text(ChildName.c_str(), window->Pos.x + 14, window->Pos.y + (text_size.y / -2) + 1, LEFT, Render::Fonts::Tahombd, false, D3DCOLOR_RGBA(15, 15, 15, g.MenuAlpha), IdaLovesMe::Vec2(0, 0), true);
	Render::Draw->Text(ChildName.c_str(), window->Pos.x + 13, window->Pos.y + (text_size.y / -2), LEFT, Render::Fonts::Tahombd, false, window->Dragging ? CMenu::get()->GetMenuColor() : D3DCOLOR_RGBA(205, 205, 205, g.MenuAlpha), IdaLovesMe::Vec2(0, 0), true);
...
void ui::TabButton(const char* label, int* selected, int num, int total, GuiFlags flags) {
	...
	Vec2 DrawPos = Window->Pos + Vec2(6, 19) + Vec2(0, num * 66); // 66 is the tab height
	...
	{
		Render::Draw->Text(label, DrawPos.x + Size.x / 2 - label_size.x / 2, DrawPos.y + Size.y / 2 - label_size.y / 2 - 1, LEFT, Render::Fonts::vsicon, false, textColor);
	}
	else if (num == 3)
	{
		Render::Draw->Text(label, DrawPos.x + Size.x / 2 - label_size.x / 2, DrawPos.y + Size.y / 2 - label_size.y / 2 - 1, LEFT, Render::Fonts::misicon, false, textColor);
	}
	else if (num == 5)
	{
		Render::Draw->Text(label, DrawPos.x + Size.x / 2 - label_size.x / 2, DrawPos.y + Size.y / 2 - label_size.y / 2 - 1, LEFT, Render::Fonts::conficon, false, textColor);
	}
	else
	{
...
void ui::TabButtonFiller(const char* label, int* selected, int num, int total, GuiFlags flags) {
	GuiContext& g = *Gui_Ctx;
	GuiWindow* Window = GetCurrentWindow();
	...
	const int sizes[] = { 69, 60 };
	const Vec2 positions[] = { Vec2(15, 17), Vec2(97, 12) };
	...
	// Draw background rectangle for tab
	Render::Draw->FilledRect(DrawPos, Size, D3DCOLOR_RGBA(12, 12, 12, g.MenuAlpha));
	Render::Draw->Line(DrawPos + Vec2(Size.x, 0), DrawPos + Vec2(Size.x, Size.y), D3DCOLOR_RGBA(40, 40, 40, g.MenuAlpha));
}
...
bool ui::Checkbox(const char* label, bool* v, bool special) {
	...
	if (*v)
		Render::Draw->Gradient(DrawPos + Vec2(1, 1), Vec2(6, 6), CMenu::get()->GetMenuColor(), D3DCOLOR_RGBA(int(EnabledColor.Value.x * 255.0f), int(EnabledColor.Value.y * 255.0f), int(EnabledColor.Value.z * 255.0f), std::clamp(180, 0, int(EnabledColor.Value.w * 255.0f))), true);
	else
		if (hovered)
...
void ui::BeginChildRagewep(const char* Name, Rect X, GuiFlags flags) {
	...
	if (!parent_window->ChildWindows.empty()) {
		...
		if (parent_window->ChildWindows.at(idx)) {
			Rect Boundaries = { (parent_window->Pos.x + 99) * 100, parent_window->Pos.y + 29, parent_window->Size.x - (99 + 23) * 100, parent_window->Size.y - (29 + 23) };

			Vec2 ChildPos = ui::GetWindowPos() + Vec2(100 * 100, 30);
			Vec2 ChildSize;
	...
	// Define the clip rectangle (the area to be clipped during rendering)
	RECT ClipRect = {
		(LONG)child_window->Pos.x,
		(LONG)child_window->Pos.y + 4,
		LONG(child_window->Pos.x + child_window->Size.x - 15),
		LONG(child_window->Pos.y + child_window->Size.y - 3)
	};
	...
	ID3D11Device* device = Render::Draw->GetD3dDevice();
	...
	Render::Draw->GetD3dDevice()->CreateRasterizerState(&rasterizerDesc, &rasterizerState);
	...
	if (!child_window->Init) {
		...
		char nigga3[20];
		char nigga4[20];
		if (!GetPrivateProfileStringA(child_window->Name.c_str(), "posx", "", nigga, 20, "C:\\menu.ini")) {

			WritePrivateProfileStringA(child_window->Name.c_str(), "posx", std::to_string(child_window->xPos).c_str(), "C:\\menu.ini");
			WritePrivateProfileStringA(child_window->Name.c_str(), "posy", std::to_string(child_window->yPos).c_str(), "C:\\menu.ini");
			WritePrivateProfileStringA(child_window->Name.c_str(), "sizex", std::to_string(child_window->xSize).c_str(), "C:\\menu.ini");
			WritePrivateProfileStringA(child_window->Name.c_str(), "sizey", std::to_string(child_window->ySize).c_str(), "C:\\menu.ini");
		}
		else {
			child_window->xPos = (int)GetPrivateProfileIntA(child_window->Name.c_str(), "posx", 0, "C:\\menu.ini");
			child_window->yPos = (int)GetPrivateProfileIntA(child_window->Name.c_str(), "posy", 0, "C:\\menu.ini");

			child_window->xSize = (int)GetPrivateProfileIntA(child_window->Name.c_str(), "sizex", 0, "C:\\menu.ini");
			child_window->ySize = (int)GetPrivateProfileIntA(child_window->Name.c_str(), "sizey", 0, "C:\\menu.ini");
		}
		child_window->Init = true;
	}
...
bool ui::SingleSelectIcon(const char* label, int* current_item, std::unordered_map<int, bool>* data, std::vector<const char*> items, LPD3DXFONT Font)
{
	...
	Vec2 display_size = { 4,4 };
	Vec2 size = Vec2(8, 8);

	//::Draw->Gradient(DrawPos + Vec2(1, 1), size - Vec2(2, 2), D3DCOLOR_RGBA(77, 77, 77, g.MenuAlpha), D3DCOLOR_RGBA(52, 52, 52, g.MenuAlpha), true);

	Render::Draw->FilledRect(Fullbb.Min + Vec2(1, 1 + 6), size - Vec2(2, 2), { 84,184,54 });
	Render::Draw->Rect(Fullbb.Min + Vec2(0, 6), size, 1, D3DCOLOR_RGBA(12, 12, 12, g.MenuAlpha));
	Render::Draw->Text(buf_display, Fullbb.Min.x + 20, Fullbb.Min.y + Fullbb.Max.y / 2.f - display_size.y / 2.f, LEFT, Render::Fonts::Verdana, false, D3DCOLOR_RGBA(192, 192, 192, g.MenuAlpha));


	Render::Draw->FilledRect(Vec2(Fullbb.Min.x + Fullbb.Max.x - 4, Fullbb.Min.y + 8), Vec2(5, 1), D3DCOLOR_RGBA(0, 0, 0, g.MenuAlpha));
	Render::Draw->FilledRect(Vec2(Fullbb.Min.x + Fullbb.Max.x - 4, Fullbb.Min.y + 9), Vec2(5, 1), D3DCOLOR_RGBA(148, 148, 148, g.MenuAlpha));
	Render::Draw->FilledRect(Vec2(Fullbb.Min.x + Fullbb.Max.x - 3, Fullbb.Min.y + 10), Vec2(3, 1), D3DCOLOR_RGBA(148, 148, 148, g.MenuAlpha));
	Render::Draw->FilledRect(Vec2(Fullbb.Min.x + Fullbb.Max.x - 2, Fullbb.Min.y + 11), Vec2(1, 1), D3DCOLOR_RGBA(148, 148, 148, g.MenuAlpha));

	std::string selected_icon = icons[*current_item];


	Vec2 icon_size = { 49, 37 };
	Render::Draw->Sprite(Render::Draw->GetRwTexture(), Vec2(Fullbb.Min.x + Fullbb.Max.x - 20 - icon_size.x, Fullbb.Min.y - 10), icon_size, D3DCOLOR_RGBA(255, 255, 255, g.MenuAlpha));
...
bool ui::SingleSelect(const char* label, int* current_item, std::vector<const char*> items) {
	if (items.empty()) return false;  // Prevent out-of-bounds access

	*current_item = std::clamp(*current_item, 0, int(items.size() - 1));

	static bool changed = false;

	const char* preview_value = (*current_item >= 0 && *current_item < items.size()) ? items[*current_item] : "N/A";

	if (ui::BeginCombo(label, preview_value, items.size())) {
		for (size_t i = 0; i < items.size(); i++) {
			if (ui::ComboSelectable(items[i], *current_item == i, GuiFlags_SingleSelect))
				*current_item = i;
		}
...
bool ui::SingleSelect(const char* label, int* current_item, std::vector<std::string> items) {
	if (items.empty()) return false;  // Prevent out-of-bounds access
	...
	*current_item = std::clamp(*current_item, 0, int(items.size() - 1));
	...
	std::string preview_value = (*current_item >= 0 && *current_item < items.size()) ? items[*current_item] : "N/A";

	if (ui::BeginCombo(label, preview_value.c_str(), items.size())) {

		for (size_t i = 0; i < items.size(); i++) {
			if (ui::ComboSelectable(items.at(i).c_str(), *current_item == i, GuiFlags_SingleSelect))
				*current_item = i;
		}
...
bool ui::KeyBind(const char* id, int* current_key, int* key_style) {
	...
	Vec2 TextSize = Render::Draw->GetTextSize(Render::Fonts::Verdana, keys[*current_key]);
	...
	Render::Draw->Text(keys[*current_key], Fullbb.Min.x + TextSize.x, Fullbb.Min.y, RIGHT, Render::Fonts::Verdana, true, TextColor);
